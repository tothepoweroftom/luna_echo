//
//  LunaExtensionBufferedAudioBus.hpp
//  LunaExtension
//
...
//  implementation's input and output audio busses.
...
struct BufferedAudioBus
{
    ...
    void initialize(AVAudioFormat* defaultFormat,
                    AVAudioChannelCount maxChannels)
    {
        maxFrames               = 0;
        pcmBuffer               = nullptr;
    ...
    void allocateRenderResources(AUAudioFrameCount inMaxFrames)
    {
        ...
        pcmBuffer = [[AVAudioPCMBuffer alloc] initWithPCMFormat:bus.format
...
/*
 ...
 This class provides a prepareOutputBufferList method to copy the internal
 buffer pointers to the output buffer list in case the client passed in null
 buffer pointers.
 */
struct BufferedOutputBus : BufferedAudioBus
{
    void prepareOutputBufferList(AudioBufferList* outBufferList,
                                 ...
                                 bool zeroFill)
    {
        ...
        for (UInt32 i = 0; i < outBufferList->mNumberBuffers; ++i)
        {
            ...
            {
                outBufferList->mBuffers[i].mData =
...
{
    /*
     Gets input data for this input by preparing the input buffer list and
     pulling the pullInputBlock.
     */
                                  ...
                                  pullInputBlock)
    {
        ...
        /*
         ...
         (inputData->mBuffers[x].mData) and mDataByteSize. mDataByteSize must be
         consistent with frameCount.

         The AURenderPullInputBlock may provide input in those specified
         buffers, or it may replace the mData pointers with pointers to memory
         which it owns and guarantees will remain valid until the next render
         cycle.
        ...
        return pullInputBlock(actionFlags,
                              timestamp,
                              frameCount,
                              inputBusNumber,
    ...
    void prepareInputBufferList(UInt32 frameCount)
    {
        ...
        mutableAudioBufferList->mNumberBuffers =
          originalAudioBufferList->mNumberBuffers;
